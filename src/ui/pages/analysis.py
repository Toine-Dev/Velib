...
from data.preprocessing import get_season_from_date  # assuming you still have this
...
def ensure_png_hourly(df, path=os.path.join(PLOT_DIR, "hourly.png")):
    ...
    sns.barplot(data=df_heure, x="heure", y="comptage_horaire", ax=ax)
...
def ensure_png_weather(df, path=os.path.join(PLOT_DIR, "weather_effects.png")):
    ...
    for c in ["pluie", "neige", "vent"]:
        if c in d.columns:
            d[c] = d[c].astype(bool)

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()
    sns.barplot(x="pluie", y="comptage_horaire", data=d, ax=axes[0]); axes[0].set_title("Pluie")
    sns.barplot(x="neige", y="comptage_horaire", data=d, ax=axes[1]); axes[1].set_title("Neige")
    sns.barplot(x="vent", y="comptage_horaire", data=d, ax=axes[2]); axes[2].set_title("Vent")
    sns.scatterplot(x="apparent_temperature", y="comptage_horaire", data=d, alpha=0.3, ax=axes[3]); axes[3].set_title("Température")
    plt.tight_layout()
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return path
...
def ensure_png_corr(df, path=os.path.join(PLOT_DIR, "corr_matrix.png")):
    if os.path.exists(path):
        return path
    cols = ["comptage_horaire", "nuit", "vacances", "heure_de_pointe", "pluie", "neige", "apparent_temperature", "vent"]
...
def ensure_png_seasons(df, path=os.path.join(PLOT_DIR, "seasons.png")):
    if os.path.exists(path):
        return path
    d = df.copy()
    d["saison"] = d["date_et_heure_de_comptage"].apply(get_season_from_date)

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()
    sns.barplot(x="saison", y="comptage_horaire", order=["winter", "spring", "summer", "autumn"], data=d, ax=axes[0]); axes[0].set_title("Saisons")
    sns.barplot(x=d["date_et_heure_de_comptage"].dt.month, y="comptage_horaire", data=d, ax=axes[1]); axes[1].set_title("Mois")
    sns.barplot(x="vacances", y="comptage_horaire", data=d, ax=axes[2]); axes[2].set_title("Vacances")
    sns.barplot(x="heure_de_pointe", y="comptage_horaire", data=d, ax=axes[3]); axes[3].set_title("Heures de pointe")
    plt.tight_layout()
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return path
...
def show_analysis():
    ...
    processed_df["date_et_heure_de_comptage"] = pd.to_datetime(processed_df["date_et_heure_de_comptage"])
    ...
    # (You can keep your statistical test sections below as-is if you want.)
...
#     processed_df = load_processed_data(raw_df_velib, raw_df_weather)
