...
""")
    ...
    try:
        ...
        st.warning("Le fichier 'compteur_paris.html' est introuvable. (Optionnel)")

    st.markdown(f"""
Cette carte présente les emplacements des bornes de comptage vélos à Paris.
Dans notre jeu de données nous avons **{raw_df_velib["identifiant_du_site_de_comptage"].nunique()} sites** de comptage.
""")
    ...
    st.markdown(f"""
Grâce à l’étape de feature engineering, nous sommes passés d'un dataset brut à un dataset prêt pour la modélisation :
- Données brutes : **{len(raw_df_velib.columns)} colonnes**, **{len(raw_df_velib):,} lignes**
- Données traitées : **{len(processed_df.columns)} colonnes**, **{len(processed_df):,} lignes**
""")
