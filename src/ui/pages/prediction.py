...
@st.cache_data
def forecast_window():
    # Forecast is generated for (max velib_raw + 1h) .. (max + 48h)
    engine = get_engine()
    last_raw = pd.read_sql("SELECT MAX(date_et_heure_de_comptage) AS dt FROM velib_raw;", engine).loc[0, "dt"]
...
def show_prediction():
    st.title("Prévisions (48h) + Heatmap")

    start_dt, end_dt = forecast_window()
    st.write(f"Fenêtre disponible : **{start_dt}** → **{end_dt}**")

    col1, col2 = st.columns(2)
    pred_date = col1.date_input("Date", min_value=start_dt.date(), max_value=end_dt.date(), value=start_dt.date())
    ...
    if dt < start_dt or dt > end_dt:
        st.error("Veuillez choisir une date/heure dans la fenêtre de prévision (48h).")
        st.stop()

    radius = st.slider("Rayon de la heatmap", min_value=10, max_value=50, value=30)

    if st.button("Afficher la heatmap"):
        df = load_forecast_geo(dt)
        ...
        heat_data = df[["latitude", "longitude", "comptage_horaire"]].dropna().values.tolist()
        HeatMap(data=heat_data, radius=radius, max_zoom=13).add_to(m)

        st.subheader("Heatmap des prévisions")
        folium_static(m)

        st.subheader("Top 15 sites")
        st.dataframe(
            df[["nom_du_site_de_comptage", "comptage_horaire"]]
